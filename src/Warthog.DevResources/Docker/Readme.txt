...
docker build -f src/Warthog.DevResources/Docker/Dockerfile.ubuntu-x64 --no-cache -t vincoss/shotlicapisvr:1.0.0-bionic .
...
## Run
docker run -it --rm -p 8001:443 --name shotlicapisvr -v shotLicData:C:/Warthog/Data vincoss/shotlicapisvr:1.0.0-windows
docker run -it --rm -p 8001:443 --name shotlicapisvr -e ASPNETCORE_URLS="https://+" -e ASPNETCORE_HTTPS_PORT=8001 -e ASPNETCORE_Kestrel__Certificates__Default__Password="p@ssword" -e ASPNETCORE_Kestrel__Certificates__Default__Path=/https/Warthog.Api.pfx -v "%USERPROFILE%/.aspnet/https:C:/https/" -v shotLicData:C:/Warthog/Data vincoss/shotlicapisvr:1.0.0-windows

## Run Windows using Linux contaners
docker run -it --rm -p 8002:443 --name shotlicapisvr -e ASPNETCORE_URLS="https://+" -e ASPNETCORE_HTTPS_PORT=8001 -e ASPNETCORE_Kestrel__Certificates__Default__Password="p@ssword" -e ASPNETCORE_Kestrel__Certificates__Default__Path=/https/Warthog.Api.pfx -v "%USERPROFILE%\.aspnet\https:/https/" -v c:/temp/shot-licensing:/var/Warthog/Data vincoss/shotlicapisvr:1.0.0-bionic
...
dotnet dev-certs https -ep "%USERPROFILE%\.aspnet\https\Warthog.Api.pfx" -p p@ssword
